...
dataOverYears = (y2024, y2025) ->
    for code, entry of y2025
        entry.byYear = { 2025: +entry.amount, 2024: +y2024[code]?amount }
        entry.change = (entry.byYear.2025 - entry.byYear.2024) / entry.byYear.2024 if entry.byYear.2024
...
init_year_data = (cb) ->
    ...
    by_year.2024 <- mapforyear 2024
    by_year.2025 <- mapforyear 2025
...
bar_chart = (id,mode) ->
    ...
    data = [{year, amount: +((by_year[year] && by_year[year][id])?amount ? 0)} for year in [2007 to 2025]]
...
test_bubble = ->
  ...
  y2024 <- mapforyear 2024
  y2025 <- mapforyear 2025
  data = dataOverYears y2024, y2025
...
testd3 = ->
    ...
    d3.select(\#y2025).on \click ->
        div.selectAll("div")
            .data treemap.value -> it.byYear?2025
        .transition()
            .duration(1500)
            .call(cell)
    d3.select(\#y2024).on \click ->
        div.selectAll("div")
            .data treemap.value -> it.byYear?2024
        .transition()
            .duration(1500)
            .call(cell)
